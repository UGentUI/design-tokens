...
const outputDir = path.join(__dirname, "../../tokens-parsed/style-dictionary");
...
fs.readFile(inputFilePath, "utf8", (err, data) => {
  ...
  const modifiedJson = transformDesignTokens(designTokensJson, false); // No need to modify references for Style Dictionary
  ...
  fs.mkdir(outputDir, { recursive: true }, (err) => {
    ...
    fs.writeFile(
      ...
      (err) => {
        if (err) throw err;
        console.log("Style Dictionary transformer");
